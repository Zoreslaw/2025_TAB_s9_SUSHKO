...
const IssuesPanel: React.FC<IssuesPanelProps> = ({ onReportIssue }) => {
  ...
  const activeIssues = issues.filter(
    (issue: ApiIssue) => issue.issueStatus === 'pending' || issue.issueStatus === 'in_progress'
  );
  
  // Calculate statistics
  const totalIssues = issues.length;
  const resolvedIssues = issues.filter((issue: ApiIssue) => issue.issueStatus === 'resolved').length;
